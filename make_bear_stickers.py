...
def umbrella(cx, top, rx=15.5, ry=9.5):
    ...
    return (f'<g stroke="{NAVY}" stroke-width="1.05" stroke-linejoin="round">'
            ...
            f'<path d="M{cx} {top+ry} V{top+ry+44}" stroke-linecap="round" fill="none"/></g>')

def sunburst(cx, cy, r=20, ink="#fff3c4"):
    import math
    rays=''.join(
...
GRADS = {
 ...
 'g-sun':  [('0%','#ffc247'),('50%','#ff9b1f'),('100%','#ef6c0a')],
}
SET=[
 dict(cap='3 BIRDIES', grad='g-bird', art=lambda cx,cy: wings(cx-13, cy-9, 46) + bear(cx+8, cy+26, 42)),
